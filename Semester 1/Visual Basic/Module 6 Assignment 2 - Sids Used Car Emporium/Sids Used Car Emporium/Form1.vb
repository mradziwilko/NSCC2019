...
Public Class Form1
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        Try
            ...
            If SelectedMan = "" Then
                Throw New Exception("No Manufacture Selected")
            End If

            If SelectedModel = "" Then
                    Throw New Exception("No Model Selected.")
                End If
            If inpQuantity.Text = "" Then
                Throw New Exception("Quantity Error: Empty.")
            End If
            If Not IsNumeric(inpQuantity.Text) Then
                Throw New Exception("Quantity Error: Numerical Numbers Only.")
            End If

            If inpQuantity.Text <= 0 Then
                Throw New Exception("Quantity Error: Invalid Quantity, Must be greater then 1.")
            End If
            ...
            Select Case SelectedModel
                ...
                Case cn_Yaris
                    ...
                    selQuantity = inpQuantity.Text
                Case Else
                    Throw New Exception("Error: Unable to add the Car to List.")
            ...
            If SelectedModel = cn_Ram1500 Or SelectedModel = cn_RoadsterCrea Then
                lbOrderCars.Items.Add($"{selMan}{vbTab}{SelectedModel}{vbTab}Qty:{selQuantity}")
            Else
                lbOrderCars.Items.Add($"{selMan}{vbTab}{SelectedModel}{vbTab}{vbTab}Qty:{selQuantity}")
            End If
            lbOrderCost.Items.Add($"{CalcCost.ToString("C2")}")
    ...
    Private Sub lbCars_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lbCars.SelectedIndexChanged
        Try
            ...
            Select Case SelectedModel
                Case cn_Focus
                    lbPrice.Text = cp_Focus.ToString("C2")
                Case cn_F150
                    lbPrice.Text = cp_F150.ToString("C2")
                Case cn_Fusion
                    lbPrice.Text = cp_Fusion.ToString("C2")
                Case cn_Escape
                    lbPrice.Text = cp_Escape.ToString("C2")
                Case cn_Tempo
                    lbPrice.Text = cp_tempo.ToString("C2")
                Case cn_Ram1500
                    lbPrice.Text = cp_Ram1500.ToString("C2")
                Case cn_Avenger
                    lbPrice.Text = cp_Avenger.ToString("C2")
                Case cn_Viper
                    lbPrice.Text = cp_Viper.ToString("C2")
                Case cn_Durango
                    lbPrice.Text = cp_Durango.ToString("C2")
                Case cn_Shadow
                    lbPrice.Text = cp_Shadow.ToString("C2")
                Case cn_Model3
                    lbPrice.Text = cp_Model3.ToString("C2")
                Case cn_ModelS
                    lbPrice.Text = cp_ModelS.ToString("C2")
                Case cn_ModelX
                    lbPrice.Text = cp_ModelX.ToString("C2")
                Case cn_RoadsterCrea
                    lbPrice.Text = cp_RoadsterCrea.ToString("C2")
                Case cn_Camry
                    lbPrice.Text = cp_Camry.ToString("C2")
                Case cn_Celica
                    lbPrice.Text = cp_Celica.ToString("C2")
                Case cn_RAV4
                    lbPrice.Text = cp_RAV4.ToString("C2")
                Case cn_Corolla
                    lbPrice.Text = cp_Corolla.ToString("C2")
                Case cn_Yaris
                    lbPrice.Text = cp_Yaris.ToString("C2")
                Case Else
                    Throw New IndexOutOfRangeException("Item Price was not found.")
            End Select


        Catch OOB As IndexOutOfRangeException
            MsgBox(OOB.Message)
            lbPrice.Text = "$"
    ...
    Sub CalcTotal()
        ...
        For i = 0 To lbOrderCost.Items.Count - 1
            Subtotal = Subtotal + lbOrderCost.Items(i)
        Next
